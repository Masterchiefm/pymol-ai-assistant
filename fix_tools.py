...
# 找到pymol_do_command的那一块并替换
new_desc = r'''执行一个或多个 PyMOL 命令。多个命令可以用换行符或分号分隔。适用于快速执行简单命令。
...
for i, line in enumerate(lines):
    if '"name": "pymol_do_command"' in line:
        in_do_command = True
    ...
    if in_do_command:
        if '"description":' in line:
            # 找到description行，替换它
            # 插入新description
            output_lines.append('                "description": ' + new_desc + ',\n')
            # 跳过原来的description和空行
            continue
        elif line.strip() == '"description":' or line.strip().startswith("执行一个或多个"):
            # 跳过旧的description内容行
            continue
        else:
